...
| `QueueDock`     | The send queue, parked on the composer's lip rather than in the transcript — so a streaming answer can't push a queued message down the page. Next-up gets the full row (send now, edit, drag-to-reorder, overflow menu); the rest collapse to one line. Hold/Resume, Combine, and Clear live in its header; given `chain` controls it also builds and runs a chain of questions; a minimize control folds it to one line, remembered per-viewer via `localStorage`. |
