...
| `Composer`      | The input: auto-growing textarea, expand toggle, attachments, mic, persona menu, send/stop. `Enter` queues while streaming; while a draft is typed mid-stream, the primary button becomes one Queue action with a hover/focus/long-press menu naming both verbs (`Queue`/`Send now`) and their keys.                                                                                                              |
| `QueueDock`     | The send queue, parked on the composer's lip rather than in the transcript — so a streaming answer can't push a queued message down the page. Next-up gets the full row (send now, edit, drag-to-reorder, overflow menu); the rest collapse to one line. Hold/Resume, Combine, and a 10s-undo Remove/Clear live in its header; a minimize control folds it to one line, remembered per-viewer via `localStorage`. |
...
| `AskedOverStrip`               | What a question was asked over, recorded on the question. A snapshot, never a view: the host passes `changed` and `onRestore`, and the strip never reads live filter state — one that did would rewrite history on every tick.                                                                                                                                                                |
...
| Hook                   | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useChat(responder)`   | Owns the conversation _and_ the send queue as two separate pieces of state — `messages` holds only turns that have actually started, `queue` holds what's waiting. Returns `{ messages, busy, queue, held, undoable, send, sendNow, stop, hold, resume, retry, editQueued, moveQueued, sendQueuedNow, combineQueue, removeQueued, clearQueue, undoQueue, reset }`, and derives each turn's `TurnTrace` from the event stream. A user turn is appended to `messages` once, at the moment it starts running, and never moves within it again — the invariant that keeps the transcript from jumping under a streaming answer. |
